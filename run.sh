...
Options:
  ...
  --test_model_path PATH          Path of model to test (optional)
  -h, --help                      Print help
EOM
...
function set_options()
{
    arguments=$(getopt --options f:h \
                       --longoptions master_port:,gpus:,config_path:,output_dir:,exp_name:,resume:,start_epoch:,test_model_path:,help \
                       --name $(basename $0) \
                       -- "$@")
...
echo -e "\tTEST_MODEL_PATH: ${TEST_MODEL_PATH}"
...
if [ $NUM_GPUS -gt 1 ]; then
    torchrun $TORCHRUN_ARGS main.py $MAIN_ARGS
else
