...
def build_metric_fn(config: dict) -> Tuple[Metric, Dict[str, float]]:
    ...
    for metric_class_name in config["target_metrics"]:
        ...
        assert hasattr(torchmetrics, metric_class_name), \
            f"Invalid metric name: {metric_class_name}"
